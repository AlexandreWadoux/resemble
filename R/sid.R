...
#'     kernel = "gaussian", 
#'     n = if(mode == "density") round(0.5 * ncol(X)), 
#'     bw = "nrd0", 
#'     reg = .Machine$double.eps^0.5)
...
#' @param reg a numerical value higher than 0 which indicates a regularization parameter. Values (probabilities) below this threshold are replaced by this value for numerical stability. Default is given by \code{.Machine$double.eps^0.5}.
#' @details
...
#' The spectral information divergence (SID) algorithm (Chang, 2000) uses the Kullback-Leibler divergence (\eqn{KL}) or relative entropy (Kullback and Leibler, 1951) to account for the vis-NIR information provided by each spectrum. The SID between two spectra (\eqn{x_{a}} and \eqn{x_{b}}) is computed as follows:
#' \deqn{
#'    SID(x_{a},x_{b}) = KL(x_{a} \left |\right | x_{b}) + KL(x_{b} \left |\right | x_{a})
#'    SID(x_{a},x_{b}) = \sum_{l=1}^{k} p_l \ log(\frac{p_l}{q_l}) + \sum_{l=1}^{k} q_l \ log(\frac{q_l}{p_l})
#' }
#' where \eqn{k} represents the number of variables or spectral features, \eqn{p} and \eqn{q} are the probability vectors of \eqn{x_{a}} and \eqn{x_{b}} respectively which are calculated as:
#' \deqn{
#'    p = \frac{x_a}{\sum_{l=1}^{k} x_{al}}
#'    q = \frac{x_b}{\sum_{l=1}^{k} x_{bl}}
#'    }
...
sid <- function(Xr, X2 = NULL, 
  ...
  if(!is.null(X2)){
    ...
    if(mode == "density"){
      ...
      X2 <- densM[(nrow(X) - nrow(X2) +1):nrow(densM), ]
      X <- densM[1:(nrow(densM) - nrow(X2)), ]
      rm(densM)
