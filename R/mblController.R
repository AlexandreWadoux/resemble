...
#'               progress = TRUE)
#' 
#' @param sm a character string indicating the spectral similarity/dissimilarity metric to be used in the selection of the nearest neighbours of each observation in \code{'Xu'} (see the \code{\link{mbl}} function). 
#'        Options are: \code{'euclid'} (Euclidean distance), \code{'cosine'} (cosine distance), \code{'sidF'} (spectral information divergence computed on the spectral variables), \code{'sidD'} (spectral information divergence computed on the density distributions of the spectra)
#'        \code{'cor'} (correlation similarity/dissimilarity), \code{'movcor'} (moving window correlation similarity/dissimilarity), \code{'pc'} (principal components distance: Mahalanobis distance computed on the principal components space) and \code{'loc.pc'} (similarity/dissimilarity estimation based on local principal components),
#'        \code{'pls'} (partial least squares distance: Mahalanobis distance computed on the partial least squares space) and \code{'loc.pls'} (similarity/dissimilarity estimation based on local partial least squares). The \code{'pc'} spectral similarity/dissimilarity metric is the default. If the \code{'sidD'} is chosen, the default parameters of the \code{sid} function are used. This argument can also be set to \code{NULL}, in such a case, a similarity/dissimilarity matrix must be specified in the \code{distanceM} argument of the \code{\link{mbl}} function.
...
#' @param ws an odd integer value which specifies the window size when the moving window correlation similarity/dissimilarity is used (i.e \code{sm = 'movcor'}). The default is 41.
...
#' @param returnDiss a logical indicating if the distance matrices must be returned.
...
#'  }
#' @return \code{mblController} returns a \code{list} of class \code{mbl} with the specified parameters
...
mblController <- function(sm = "pc",
  ...
  if(sm == "none")
  {
    message("'sm' was set to 'none'. You will have to provide a distance matrix to the mbl function by using the 'distanceM' argument")
  }else{
