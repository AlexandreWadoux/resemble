...
#' @usage 
#' simEval(d, sideInf, ...)
#' @param d a square symmetric \code{matrix} (or \code{data.frame}) of similarity/dissimilarity scores between samples of a given dataset.
...
#' pca.d <- orthoDiss(Xr = Xr, pcSelection = list("cummvar", 0.999), 
#'                    method = "pca", 
#'                    local = FALSE, 
...
simEval <- function(d, sideInf, ...){
  ...
  if(is.numeric(as.matrix(sideInf))){
    ...
    if(ny >1){
      mn.s <- data.frame(mn.sd.rmsd = mean(rslt$rmsd/cSds(sideInf)), mn.r = mean(rslt$r))
